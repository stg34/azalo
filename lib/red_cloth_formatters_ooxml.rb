...
module RedCloth
  module Formatters
    module IntermediateOOXml

      #openssl rand -base64 32
      BreakStr = 'BREAK--------VtTjZRu66RIYPhnqDNpgjyfme3JkrVYIpU3PEC9Eluk='
      ...
      def footno(opts)
        opts[:id] ||= opts[:text]
        %Q{<footnoteref id=\"fnr#{opts[:id]}\">#{opts[:id]}</footnoteref>}
      end
      ...
      def notextile(opts)
        if filter_html
          '<p>' + html_esc(opts[:text], :html_escape_preformatted) + '</p>'
        else
          '<p>' + opts[:text] + '</p>'
        end
      end
  ...
  class TextileDoc
    def to_intermediate_oo_xml(*rules)
      apply_rules(rules)
      output = to(Formatters::IntermediateOOXml)
