...
module ExceptionNotification::NotifierHelper
  ...
  def render_overridable(partial, options={})
    ...
    if File.exist?(path = "#{APP_PATH}/_#{partial}.html.erb") ||
      ...
      puts path 
      puts partial
      #p options.merge(:file => path)
      puts '7 -----------------------------'
      #o = options.merge(:file => path, :use_full_path => true)
      o = options.merge(:file => "#{partial}.html", :use_full_path => true)
      return render({:partial => partial}, o)
    else
